...
TEST_CASE("TypeConstraintVisitor: function reference, address of",
  ...
  program << R"(
    ...
    auto yType = std::make_shared<TopVar>(symbols->getLocal("y", fDecl));
    auto expectedBorrow = std::make_shared<TopBorrowRef>(
      TypeHelper::funType(empty, TypeHelper::intType()));
    REQUIRE(*unifier.inferred(yType) == *expectedBorrow);
...
TEST_CASE("TypeConstraintVisitor: polymorphic type inference",
  ...
  std::stringstream program;
  program << R"(
      // [[p]] = ref&\alpha, [[deref]] = (ref&\alpha) -> \alpha
      deref(p) {
        return *p;
      }
    )";
    ...
    auto refType = std::dynamic_pointer_cast<ReferenceType>(funType->getParamTypes()[0]);
    REQUIRE(refType != nullptr);
    REQUIRE(std::dynamic_pointer_cast<TopModeVar>(refType->getMode()) != nullptr);
