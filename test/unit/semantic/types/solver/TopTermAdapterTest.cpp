...
TEST_CASE("TopTermAdapter: isVariable false for TopRef", "[TopTermAdapter]") {
  auto inner = std::make_shared<TopInt>();
  REQUIRE_FALSE(TopTermAdapter::wrap(std::make_shared<TopRef>(inner))->isVariable());
}
...
TEST_CASE("TopTermAdapter: getFunctor and arity for TopRef", "[TopTermAdapter]") {
  auto ref = std::make_shared<TopRef>(std::make_shared<TopInt>());
...
TEST_CASE("TopTermAdapter: getSubterms for TopRef", "[TopTermAdapter]") {
  auto inner = std::make_shared<TopInt>();
  auto ref = std::make_shared<TopRef>(inner);
  auto subs = TopTermAdapter::wrap(ref)->getSubterms();
  REQUIRE(subs.size() == 1);
  auto recovered = TopTermAdapter::unwrap(subs[0]);
...
TEST_CASE("TopTermAdapter: round-trip withSubterms for TopRef",
          "[TopTermAdapter]") {
  auto inner = std::make_shared<TopInt>();
  auto ref = std::make_shared<TopRef>(inner);
  auto adapter = TopTermAdapter::wrap(ref);
