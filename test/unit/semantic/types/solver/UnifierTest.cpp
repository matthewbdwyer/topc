...
TEST_CASE("Unifier: Collect and then unify constraints",
  ...
  SECTION("Test type-safe program 1") {
    ...
    program << R"(
            // x is int, y is own&int, z is int, short is () -> int
            short() {
              var x, y, z;
              x = input;	
              y = alloc x;
    ...
    auto ptrToInt = std::make_shared<TopOwningRef>(intType);
  ...
  SECTION("Test type-safe deref") {
    std::stringstream program;
    program << R"(
// deref is (ref&\alpha<*p>) -> \alpha<*p>, p is ref&\alpha<*p>
deref(p){
    return *p;
}
         )";
    ...
    auto polyArgRef = std::dynamic_pointer_cast<ReferenceType>(polyArg);
    REQUIRE(polyArgRef != nullptr);
    REQUIRE(std::dynamic_pointer_cast<TopModeVar>(polyArgRef->getMode()) != nullptr);
