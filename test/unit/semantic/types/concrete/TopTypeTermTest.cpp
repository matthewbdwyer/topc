...
// ==================== TopOwningRef Term Interface ====================

TEST_CASE("TopOwningRef: Term interface - isVariable", "[TopType][Term]") {
  auto inner = std::make_shared<TopInt>();
  auto t = std::make_shared<TopOwningRef>(inner);
  REQUIRE_FALSE(t->isVariable());
}

TEST_CASE("TopOwningRef: Term interface - getFunctor", "[TopType][Term]") {
  auto inner = std::make_shared<TopInt>();
  auto t = std::make_shared<TopOwningRef>(inner);
  REQUIRE(t->getFunctor() == "ref");
}

TEST_CASE("TopOwningRef: Term interface - arity", "[TopType][Term]") {
  auto inner = std::make_shared<TopInt>();
  auto t = std::make_shared<TopOwningRef>(inner);
  REQUIRE(t->arity() == 2);
}

TEST_CASE("TopOwningRef: Term interface - getSubterms", "[TopType][Term]") {
  auto inner = std::make_shared<TopInt>();
  auto t = std::make_shared<TopOwningRef>(inner);
...
TEST_CASE("TopOwningRef: Term interface - withSubterms", "[TopType][Term]") {
  auto inner = std::make_shared<TopInt>();
  auto t = std::make_shared<TopOwningRef>(inner);
  auto newInner = std::make_shared<TopOwningRef>(inner);
  auto t2 = t->withChildTypes({t->getMode(), newInner});
  REQUIRE(t2->getFunctor() == "ref");
  auto subs = t2->getChildTypes();
  REQUIRE(subs.size() == 2);
  REQUIRE(subs[0]->getFunctor() == "ownmode");
  REQUIRE(subs[1]->getFunctor() == "ref");
}

TEST_CASE("TopOwningRef: Term interface - withSubterms wrong count throws", "[TopType][Term]") {
  auto inner = std::make_shared<TopInt>();
  auto t = std::make_shared<TopOwningRef>(inner);
  REQUIRE_THROWS_AS(t->withChildTypes({}), std::invalid_argument);
  REQUIRE_THROWS_AS(t->withChildTypes({inner}), std::invalid_argument);
}
...
TEST_CASE("TopFunction: Term interface - getSubterms", "[TopType][Term]") {
  auto intType = std::make_shared<TopInt>();
  auto ptrType = std::make_shared<TopOwningRef>(intType);
  auto func = std::make_shared<TopFunction>(
...
TEST_CASE("TopMu: Term interface - isVariable", "[TopType][Term]") {
  auto node = std::make_unique<ASTNumberExpr>(42);
  auto alpha = std::make_shared<TopAlpha>(node.get(), "0");
  auto body = std::make_shared<TopOwningRef>(alpha);
...
TEST_CASE("Term equals: TopOwningRef with same inner are equal", "[TopType][Term]") {
  auto inner1 = std::make_shared<TopInt>();
  auto inner2 = std::make_shared<TopInt>();
  auto ref1 = std::make_shared<TopOwningRef>(inner1);
  auto ref2 = std::make_shared<TopOwningRef>(inner2);
  REQUIRE(*ref1 == *ref2);
}
...
TEST_CASE("Term matchesFunctor: different functor", "[TopType][Term]") {
  auto t1 = std::make_shared<TopInt>();
  auto t2 = std::make_shared<TopOwningRef>(t1);
  REQUIRE(t1->getFunctor() != t2->getFunctor());
}
