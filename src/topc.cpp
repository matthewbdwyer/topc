...
static cl::opt<bool> ppretty("pp", cl::desc("pretty print"), cl::cat(TOPcat));
static cl::opt<bool> psym("ps", cl::desc("print symbols"), cl::cat(TOPcat));
static cl::opt<bool>
...
static cl::opt<std::string>
    astFile("pa", cl::value_desc("AST output file"),
            cl::desc("print AST to a file in dot syntax"), cl::cat(TOPcat));
static cl::opt<std::string>
  ...
  try {
    ...
    try {
      ...
      if (printCG) {
        ...
        if (!cgStream.good()) {
          LOG_S(ERROR) << "topc: error: failed to open '" << cgFile
                       << "' for writing";
          std::exit(EXIT_FAILURE);
        }
      ...
      if (printAST) {
        ...
        if (!astStream.good()) {
          LOG_S(ERROR) << "topc: error: failed to open '" << astFile
                       << "' for writing";
        } else {
