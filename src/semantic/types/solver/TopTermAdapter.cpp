...
bool TopTermAdapter::isVariable() const {
  ...
  return topType->isVariable() ||
         std::dynamic_pointer_cast<TopVar>(topType) != nullptr;
}
...
bool TopTermAdapter::matchesFunctor(const Term &other) const {
  return Term::matchesFunctor(other);
}

bool TopTermAdapter::preferAsRepresentativeOver(const Term &other) const {
  return false;
}

bool TopTermAdapter::unifiesByStructureOnlyWith(const Term &other) const {
  return false;
}
