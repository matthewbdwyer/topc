...
BUILD_DOCS_DIR="${ROOT_DIR}/build/docs"
...
echo
echo "Documentation has been generated and can be found in ${BUILD_DOCS_DIR}"
echo "You can view the html by opening ${BUILD_DOCS_DIR}/html/index.html in a browser"
echo

# Check if coverxygen is installed
if ! python3 -m pip show coverxygen > /dev/null 2>&1; then
...
python3 -m coverxygen \
  ...
  --kind class \
  --scope public | \
  jq '{"schemaVersion": 1, "label": "doxygen", "color":"informational", "message": "\((.total.coverage_rate*100 | floor))%"}' > ../build/docs/html/doccoverage.json
